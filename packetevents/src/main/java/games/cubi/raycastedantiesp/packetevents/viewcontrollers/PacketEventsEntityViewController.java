...
public abstract class PacketEventsEntityViewController extends PacketEntityViewController<PacketWrapper<?>> implements PacketListener {
    ...
    /**
     * Decides what to do with a packet whose entity is not tracked.
     * <p>
     * Minecraft can send entity state before the matching spawn packet, so such an update is normally deferred until
     * that spawn arrives. An entity the player left behind in their previous world will never spawn again though, and
     * its straggling packets would otherwise sit in the queue until they expired and were reported as evicted.
     *
     * @return true if the caller should queue a deferred task, false if the packet should be dropped.
     */
    private static boolean shouldDeferUntilSpawn(PlayerData playerData, int entityID, String packetDescription, int currentTick) {
        if (playerData.nettyData().isPreviousWorldEntity(entityID, currentTick)) {
            Logger.info("Dropped " + packetDescription + " packet for an entity left behind in the previous world, id="
                    + entityID + ".", 7, PacketEventsEntityViewController.class);
            return false;
        }
        Logger.warning("Received " + packetDescription + " packet for unknown entity, id=" + entityID
                + ". Queuing retry.", 6, PacketEventsEntityViewController.class);
        return true;
    }
    ...
    protected int processRelativeMovePacket(PacketWrapper<?> packetWrapper, PlayerData playerData, int currentTick) {
        ...
        if (entity == null) {
            if (shouldDeferUntilSpawn(playerData, entityID, "relative move", currentTick)) {
    ...
    protected int processRelativeMoveAndRotationPacket(PacketWrapper<?> packet, PlayerData playerData, int currentTick) {
        ...
        NettyEntity<?> entity = playerData.entityFromID(entityID);
        if (entity == null) {
            if (shouldDeferUntilSpawn(playerData, entityID, "relative move and rotation", currentTick)) {
    ...
    protected int processTeleportPacket(PacketWrapper<?> packet, PlayerData playerData, int currentTick) {
        ...
        NettyEntity<?> entity = playerData.entityFromID(entityID);
        if (entity == null) {
            if (shouldDeferUntilSpawn(playerData, entityID, "teleport", currentTick)) {
    ...
    protected int processPositionSyncPacket(PacketWrapper<?> packet, PlayerData playerData, int currentTick) {
        ...
        NettyEntity<?> entity = playerData.entityFromID(entityID);
        if (entity == null) {
            if (shouldDeferUntilSpawn(playerData, entityID, "position sync", currentTick)) {
    ...
    protected void cachePacket(PacketWrapper<?> packet, int entityID, PlayerData playerData, int currentTick) {
        ...
        NettyEntity<?> entity = playerData.entityFromID(entityID);
        if (entity == null) {
            if (shouldDeferUntilSpawn(playerData, entityID, "cacheable " + packet.getClass().getSimpleName(), currentTick)) {
    ...
    protected int processRotationPacket(PacketWrapper<?> packet, PlayerData playerData, int currentTick) {
        ...
        NettyEntity<?> entity = playerData.entityFromID(entityID);
        if (entity == null) {
            if (shouldDeferUntilSpawn(playerData, entityID, "rotation", currentTick)) {
    ...
    protected int processHeadLookPacket(PacketWrapper<?> packet, PlayerData playerData, int currentTick) {
        ...
        NettyEntity<?> entity = playerData.entityFromID(entityID);
        if (entity == null) {
            if (shouldDeferUntilSpawn(playerData, entityID, "head look", currentTick)) {
    ...
    protected int processEntityVelocityPacket(PacketWrapper<?> packet, PlayerData playerData, int currentTick) {
        ...
        NettyEntity<?> entity = playerData.entityFromID(entityID);
        if (entity == null) {
            if (shouldDeferUntilSpawn(playerData, entityID, "velocity", currentTick)) {
