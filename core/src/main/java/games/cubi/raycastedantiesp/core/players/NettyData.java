...
public class NettyData implements Clearable {
    ...
    /**
     * The entities the player left behind in their previous world.
     * <p>
     * Held separately from {@link #expectedWorldTransitionDestroyEntityIDs} because that list is consumed by the
     * destroy packets, so it can already be empty by the time a straggling packet for one of those entities arrives.
     */
    private int @IntArrayListMarker [] previousWorldEntityIDs;
    private int worldTransitionTick;

    /**
     * Records which entities belonged to the world the player just left. Packets which were already in the outbound
     * pipeline when the world changed can then be told apart from packets that merely arrived before their spawn
     * packet.
     */
    public void recordWorldTransition(int[] previousEntityIDs, int currentTick) {
        previousWorldEntityIDs = PrimitiveIntArrayList.isEmpty(previousEntityIDs) ? null : previousEntityIDs.clone();
        worldTransitionTick = currentTick;
    }

    /**
     * @return whether this entity belonged to the world the player just left, meaning no spawn packet for it can ever
     * arrive. Only reports true briefly after a world change, so an entity ID reused by the new world cannot be
     * mistaken for a stale one.
     */
    public boolean isPreviousWorldEntity(int entityID, int currentTick) {
        if (previousWorldEntityIDs == null) {
            return false;
        }
        if (currentTick - worldTransitionTick >= EntitySpawnTask.TICKS_BEFORE_EVICTION) {
            // Any straggler would already have expired by now, so stop scanning this list.
            previousWorldEntityIDs = null;
            return false;
        }
        return PrimitiveIntArrayList.contains(previousWorldEntityIDs, entityID);
    }
    //
