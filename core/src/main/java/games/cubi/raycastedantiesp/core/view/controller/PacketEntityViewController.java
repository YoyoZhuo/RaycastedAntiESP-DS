...
public abstract class PacketEntityViewController<P> {
    ...
    protected void handleWorldStatePacket(UUID player, String world, UUID worldUUID, int minWorldHeight, int currentTick) {
        ...
        try {
            if (previousWorld != null) {
                ...
                // Recorded after the clear above, which wipes the rest of the deferred reconciliation state.
                nettyData.recordWorldTransition(remainingEntityIDs, currentTick);
    ...
    protected void handleDestroyEntities(int[] entityIDs, PlayerData playerData, int currentTick) {
        for (int entityID : entityIDs) {
            ...
            playerData.nettyData().clearPendingPostSpawnTasksForEntity(entityID);
            // Bypassed entities are never tracked, and an already-removed entity can receive a second destroy packet.
            // Both are routine, so this lookup must not report the miss.
            EntityView<?> view = playerData.viewFromEntityIDIfTracked(entityID);
    ...
    private void clearPassengerReferencesForDestroyedEntity(int entityID, PlayerData playerData) {
        ...
        if (!PrimitiveIntArrayList.isEmpty(unresolvedPassengers)) {
            for (int passengerID : unresolvedPassengers) {
                NettyEntity<?> passenger = playerData.entityFromIDIfTracked(passengerID);
        ...
        if (unresolvedVehicleID != NO_VEHICLE) {
            NettyEntity<?> unresolvedVehicle = playerData.entityFromIDIfTracked(unresolvedVehicleID);
        ...
        NettyEntity<?> entity = playerData.entityFromIDIfTracked(entityID);
        if (entity == null) {
            return;
        }
        ...
        NettyEntity<?> vehicle = playerData.entityFromIDIfTracked(vehicleID);
        if (vehicle != null) {
    ...
    private void clearLeashReferencesForDestroyedEntity(int entityID, PlayerData playerData) {
        ...
        if (!PrimitiveIntArrayList.isEmpty(pendingLeashedEntityIDs)) {
            for (int leashedEntityID : pendingLeashedEntityIDs) {
                NettyEntity<?> leashedEntity = playerData.entityFromIDIfTracked(leashedEntityID);
        ...
        if (pendingHolderEntityID != NO_LEASHER) {
            NettyEntity<?> pendingHolder = playerData.entityFromIDIfTracked(pendingHolderEntityID);
        ...
        if (holderEntityID != NO_LEASHER) {
            NettyEntity<?> holder = playerData.entityFromIDIfTracked(holderEntityID);
            if (holder != null) {
